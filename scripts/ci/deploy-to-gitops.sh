...
# Invoked via `bash` rather than executed directly so this doesn't depend
# on the executable bit surviving checkout/clone — that bit has been
# inconsistent in CI (permission denied even after an explicit chmod +x
# step), and `bash <file>` works regardless of the file's mode as long as
# it's readable.
bash "${SCRIPT_DIR}/update-image-tag.sh" "${GITOPS_PATH}" "${ENVIRONMENT}" "${IMAGE_TAG}"
